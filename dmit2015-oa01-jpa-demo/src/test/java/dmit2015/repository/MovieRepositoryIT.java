...
@TestMethodOrder(MethodOrderer.OrderAnnotation.class) // These 2 lines execute the methods in sequent
...
//                .addAsLibraries(pomFile.resolve("org.hibernate:hibernate-core-jakarta:5.6.5.Final").withTransitivity().asFile())
    ...
    void shouldFailToCreate(){
        ...
        Exception exeption = assertThrows(ConstraintViolationException.class, () -> {
            _movieRepository.add(emptyMovie);
        });
        assertTrue(exeption.getMessage().contains("The Title field is required."));
        //assertTrue(exeption.getMessage().contains("The field Title must be a string with a minimum length of {min} and a maximum length of {max}."));
        assertTrue(exeption.getMessage().contains("The Release Date field is required"));
        assertTrue(exeption.getMessage().contains("The field Genre is required."));
        assertTrue(exeption.getMessage().contains("The field Rating is required."));
    }
